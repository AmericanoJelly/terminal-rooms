...
function setView(name) {
  for (const key of ['welcomeView', 'joinView', 'chatView']) {
    el[key].classList.toggle('active', key === name);
  }
}
...
async function initSession() {
  let stored = localStorage.getItem('terminal_rooms_client_id');
...
async function createRoom() {
  ...
  if (!name) {
    el.createStatus.textContent = 'room name required';
    return;
  }
  try {
    const { room } = await api('/api/rooms', {
      method: 'POST',
      body: JSON.stringify({ name, clientId: state.clientId, nickname }),
    });
...
async function joinRoomFromPrompt() {
  ...
  if (!nickname) {
    el.joinStatus.textContent = 'nickname required';
    return;
  }
  try {
...
async function enterChat(roomId, nickname) {
  const [{ room }, { messages }] = await Promise.all([
    api(`/api/rooms/${roomId}?clientId=${encodeURIComponent(state.clientId)}`),
...
function appendMessage(msg) {
  ...
  if (msg.type === 'system') {
    div.innerHTML = `<span class="time">[${fmtTime(msg.createdAt)}]</span> system &gt; ${escapeHtml(msg.content)}`;
  } else {
    div.innerHTML = `<span class="time">[${fmtTime(msg.createdAt)}]</span> <span class="name">${escapeHtml(msg.senderNickname)}</span> &gt; ${escapeHtml(msg.content)}`;
  }
...
function connectSocket(roomId, nickname) {
  ...
  state.socket.on('room:error', ({ message }) => {
    el.chatStatus.textContent = message;
  });
  state.socket.on('disconnect', () => {
    el.chatStatus.textContent = 'connection lost';
  });
}
...
async function leaveCurrentRoom() {
  ...
  try {
    await api(`/api/rooms/${state.currentRoom.id}/leave`, {
      method: 'POST',
      body: JSON.stringify({ clientId: state.clientId }),
    });
    if (state.socket) {
      state.socket.emit('room:leave', { roomId: state.currentRoom.id, clientId: state.clientId });
    }
  } catch (_) {
    // ignore
  }
